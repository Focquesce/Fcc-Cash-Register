...
function checkCashRegister(price, cash, cid) {
  ...
  const denominaeObj = cidToObj(denominae);
  let res = { status: '', change: []};  
  let updatedCidObj = {};
  let updatedChange = cash - price;

  for (let unit of Object.keys(cidObj) ) {
    let tempResAmount = Math.floor(updatedChange / denominaeObj[unit]);
    let currentAmount = Math.round(cidObj[unit] / denominaeObj[unit]);

    let resAmount = currentAmount <= tempResAmount ? currentAmount : tempResAmount;
    let resAmountValue = resAmount * denominaeObj[unit];
...
checkCashRegister(19.5, 500, denom) ;
